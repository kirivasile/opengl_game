...
void OrbitCameraMover::update(GLFWwindow* window, double dt)
{
	...
	//¬ычисл€ем положение виртуальной камеры в мировой системе координат по формуле сферических координат
	...
	//ќбновл€ем матрицу проекции на случай, если размеры окна изменились
...
FreeCameraMover::FreeCameraMover() :
	...
	_pos(5.0f, 0.0f, 2.5f)
{
	//Ќам нужно как-нибудь посчитать начальную ориентацию камеры
...
void FreeCameraMover::update(GLFWwindow* window, double dt)
{
	...
	//ѕолучаем текущее направление "вперед" в мировой системе координат
	glm::vec3 forwDir = glm::vec3(0.0f, 0.0f, -1.0f) * _rot;

	//ѕолучаем текущее направление "вправо" в мировой системе координат
	glm::vec3 rightDir = glm::vec3(1.0f, 0.0f, 0.0f) * _rot;

	//ƒвигаем камеру вперед/назад
	if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS)
	{
	...
	//—оедин€ем перемещение и поворот вместе
