...
const std::string Texture::TEXTURES_DIR = "mainData/textures/";
...
TexturePtr loadTexture(const std::string& filename, SRGB srgb)
{
	...
	std::string filepath = Texture::TEXTURES_DIR + filename;
...
{
	std::string filepath = Texture::TEXTURES_DIR + filename;
    GLuint tex = SOIL_load_OGL_texture(filepath.c_str(), SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_DDS_LOAD_DIRECT);
    if (tex == 0)
    {
        std::cerr << "SOIL loading error: " << SOIL_last_result() << std::endl;
        return std::make_shared<Texture>();
...
TexturePtr loadCubeTexture(const std::string& basefilename)
{
	...
	std::string baseFilePath = Texture::TEXTURES_DIR + basefilename;
    loadCubeTextureFace(texture, GL_TEXTURE_CUBE_MAP_NEGATIVE_X, baseFilePath + "/negx.png");
    loadCubeTextureFace(texture, GL_TEXTURE_CUBE_MAP_POSITIVE_X, baseFilePath + "/posx.png");
    loadCubeTextureFace(texture, GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, baseFilePath + "/negy.png");
    loadCubeTextureFace(texture, GL_TEXTURE_CUBE_MAP_POSITIVE_Y, baseFilePath + "/posy.png");
    loadCubeTextureFace(texture, GL_TEXTURE_CUBE_MAP_NEGATIVE_Z, baseFilePath + "/negz.png");
    loadCubeTextureFace(texture, GL_TEXTURE_CUBE_MAP_POSITIVE_Z, baseFilePath + "/posz.png");
